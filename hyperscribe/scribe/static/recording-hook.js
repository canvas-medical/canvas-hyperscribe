...
// Finish-time catch-up: after a connectivity drop, minutes of audio can still
// be buffered locally. On Finish we must let that backlog drain (send + ACK)
// before closing, or the tail of the transcript is silently lost. Poll the
// client's pending-audio gauge; keep waiting while it's shrinking, bail only
// if it stalls (network died again) or a generous hard cap is hit.
...
export function useRecording(noteId, initialTranscript, { mode = 'conversation' } = {}) {
  ...
  // Seconds of buffered audio still catching up after Finish (0 when drained).
  ...
  // True while pauseRecording's drainBeforeClose is in flight (KOALA-5934).
  // Guards resumeRecording: connectAndRecord() would overwrite clientRef.current
  // mid-drain, causing the old drain loop to read the new client's near-zero
  // pending audio and exit as falsely 'drained', silently losing the old
  // client's un-transcribed buffer. Resume is also disabled in the UI while
  // this is true (see awaitingTranscription), but this ref is the hard guard.
  const pauseDrainingRef = useRef(false);
  ...
  // tune FINISH_WAIT_MS empirically from real sessions.
