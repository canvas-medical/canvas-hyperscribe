...
class RubricGenerator:
    ...
    def build_prompt(self, transcript: list[dict[str, Any]],
        ...
        self.llm.set_user_prompt([
                ...
                "decisions or actions — your role is not to evaluate what should have been done, "
                "only what was actually stated and how well that is reflected in the note.",
                
                "The scribe's job is not to correct clinical mistakes, infer intentions, or introduce new content. "
                "The rubric must reward or penalize the documentation *solely* based on whether it captured what was "
                "*explicitly or implicitly* stated in the transcript, and whether it leveraged relevant details from the chart without copying irrelevant ones.",
                "The transcript is the source of truth — even if it contains questionable decisions, you must "
                "assume they were spoken and should be documented accordingly. Do not bring in external clinical knowledge "
                "or judge decisions using best practices or safety standards. Only consider fidelity to what is said and known.",
                
                "You must follow the following three-step reasoning process before writing the rubric:",
                
                "**Step 1: Identify key clinical events, statements, decisions, or contextual facts** that appear in the transcript and/or chart. "
                "These may include diagnoses discussed, medications prescribed, reasoning offered, patient symptoms or preferences, "
                "and social or medical history relevant to the encounter.",
                
                "**Step 2: From those, identify what an ideal ambient scribe should have captured in the documentation** — "
                "including specific statements, decisions, reasoning, historical facts, or numerical details "
                "(e.g., names, dates, dosages) that are crucial to the clinical narrative. "
                "Be strict: only include elements verifiable from the transcript or chart.",
                
                "**Step 3: Using Step 2, write the rubric as a JSON array of criteria**."
                "Each criterion must reflect whether the note faithfully documents a specific item from Step 2. "
                "Do not add criteria that involve external knowledge, best practices, or normative clinical expectations.",
                
                "Each criterion must be formatted as: {\"criterion\": <clear, verifiable fidelity-focused criterion>," 
                "\"weight\": <int from 0–100>, \"sense\": \"positive\"|\"negative\"}.",
                ...
                "Include at least one criterion that addresses overall completeness of the note relative to the transcript, "
                "and at least one that addresses selective or inaccurate copying from the chart.",
                "Avoid redundancy or generality. Focus on concrete, case-specific documentation fidelity.",
                
                "Only return the final rubric JSON array as your output. Do not return steps 1 or 2 explicitly in the output. "
                "Do not include markdown, commentary, or text wrapping. Output must begin with `[` and end with `]`.",
