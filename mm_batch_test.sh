...
for patient_dir in $(ls -d "$CATEGORY_DIR"/Patient_* | sort -t '_' -k2 -n); do
    ...
    uv run python evaluations/cases/synthetic_unit_cases/grader.py \
        "$patient_dir/rubric.json" \
        "$patient_dir/note.json" \
        "$patient_dir/scores.json"
    
    uv run python evaluations/cases/synthetic_unit_cases/eval_report.py \
        "$patient_dir/rubric.json" \
        "$patient_dir/scores.json" \
        "$patient_dir/eval_report_o3.csv"

    echo "Generation, evaluation and evaluation report complete for $patient_dir."
    done
