...
class InvestigateFeedback:
    ...
    @classmethod
    def _extract_iso_date(cls, transcript_objects: list[Any]) -> str | None:
        """Extract ISO date from the first transcript object's last modified date."""
        if not transcript_objects:
            return None
        return cast(str, transcript_objects[0].last_modified.strftime("%Y-%m-%d"))

    @classmethod
    def _retrieve_transcript_objects(cls, s3_client: AwsS3, prefix: str) -> list[Any]:
        """Retrieve all transcript objects from S3 under the given prefix."""
        return s3_client.list_s3_objects(prefix)

    @classmethod
    def _retrieve_log_objects(cls, s3_client: AwsS3, prefix: str, note_id: str) -> list[Any]:
        """
    ...
    @classmethod
    def _parse_and_concatenate_transcripts(
        cls, s3_client: AwsS3, transcript_objects: list[Any], debug: bool = False
    ) -> list[dict]:
        """
    ...
    @classmethod
    def _parse_and_concatenate_logs(cls, s3_client: AwsS3, log_objects: list[Any], debug: bool = False) -> list[str]:
        """
