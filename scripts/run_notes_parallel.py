...
class RunNotesParallel:
    ...
    @classmethod
    def parameters(cls):
        parser = ArgumentParser(description="Run note grading in parallel for a specific case")
        parser.add_argument("--case", type=int, required=True, help="Case ID to process")
        parser.add_argument("--max-workers", type=int, default=2, help="Maximum number of parallel workers")
        parser.add_argument("--delay", type=float, default=1.0, help="Delay between task submissions in seconds")
        return parser.parse_args()

    def load_notes_for_case(self) -> list[dict]:
        """Load all notes for the specified case ID."""
        notes = []
        with open(self.experiment_notes_csv, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                if int(row['case_id']) == self.case_id:
                    notes.append({
    ...
    def load_rubrics_for_case(self) -> list[dict]:
        """Load all rubric IDs for the specified case ID."""
        rubrics = []
    ...
    def generate_tasks(self) -> list[NoteRubricPair]:
        ...
        notes = self.load_notes_for_case()
        rubrics = self.load_rubrics_for_case()
        
        if not notes:
            raise ValueError(f"No notes found for case {self.case_id}")
        if not rubrics:
            raise ValueError(f"No rubrics found for case {self.case_id}")
    ...
    def run_parallel(self, max_workers: int = 8) -> None:
        ...
        print(f"Generated {len(tasks)} grading tasks for case {self.case_id}")
        print(f"Notes: {len(self.load_notes_for_case())}")
        print(f"Rubrics: {len(self.load_rubrics_for_case())}")
