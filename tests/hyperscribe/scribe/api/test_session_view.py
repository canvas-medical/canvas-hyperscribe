...
def test_summary_js_diagnose_to_assess_upstream_flip() -> None:
    ...
    Three structural pins (all required):
      ...
      2. A ``tagged = tagged.map(`` assignment carrying a diagnose→assess
         transform appears BEFORE the ``/insert-commands`` POST. (PR #257
         refactored the stamping path to an origin-tracked pipeline where
         the flip operates on the ``tagged`` array rather than directly on
         ``workingCommands``; the structural guarantee — flip-before-POST
         — is preserved.)
    ...
    # (2) The upstream flip must happen before the /insert-commands POST.
    # The origin-tracked pipeline (PR #257) flips on the `tagged` array
    # rather than directly on `workingCommands`; the load-bearing invariant
    # is unchanged — the row's command_type is 'assess' by the time it
    # reaches /insert-commands.
    flip_marker_idx = src.find("KOALA_5634_DIAGNOSE_TO_ASSESS_UPSTREAM_FLIP")
    insert_post_decl = "`${API_BASE}/insert-commands`"
    insert_post_idx = src.find(insert_post_decl)
    assert insert_post_idx != -1, (
        "Expected to find the `/insert-commands` POST in summary.js. "
        "If the endpoint URL or fetch shape changed, update this pin."
    )
    assert flip_marker_idx < insert_post_idx, (
        "The KOALA_5634_DIAGNOSE_TO_ASSESS_UPSTREAM_FLIP block must appear "
        "BEFORE the /insert-commands POST. Running the flip after the POST "
        "means the canonical stamp key still says `diagnose`, the canonical "
        "stamp key misses, and the row duplicates on the next "
        "syncNoteCommands (KOALA-5634)."
    )
    flip_window = src[flip_marker_idx:insert_post_idx]
    flip_assignment_pattern = re.compile(
        r"tagged\s*=\s*tagged\.map\(",
    )
    assert flip_assignment_pattern.search(flip_window) is not None, (
        "Expected `tagged = tagged.map(...)` inside the "
        "KOALA_5634_DIAGNOSE_TO_ASSESS_UPSTREAM_FLIP block. The flip must "
        "actually mutate the tagged array so the row's command_type "
        "becomes `assess` before the /insert-commands POST runs (KOALA-5634)."
    )
    ...
    assert assess_flip_pattern.search(flip_window) is not None, (
        ...
        "row stays as `diagnose` and the canonical stamp key still "
        "misses (KOALA-5634)."
    )
