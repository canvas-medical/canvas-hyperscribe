...
def test_handle(launch_modal_effect: object, mock_note: MagicMock, mock_hide: MagicMock) -> None:
    ...
    tested = ScribeApp(event, {"ScribeHideChartButtons": "true"})
...
@patch("hyperscribe.scribe.application.transcript_app.Note")
@patch("hyperscribe.scribe.application.transcript_app.LaunchModalEffect")
def test_handle_without_button_hiding(launch_modal_effect: object, mock_note: MagicMock, mock_hide: MagicMock) -> None:
    """Without ScribeHideChartButtons the tab opens and the chart rail is left alone."""
    launch_modal_effect.return_value.apply.side_effect = [  # type: ignore[union-attr]
        Effect(type="LOG", payload="SomePayload")
    ]
    launch_modal_effect.TargetType.NOTE = "note"  # type: ignore[union-attr]
...
@patch("hyperscribe.scribe.application.transcript_app.Note")
@patch("hyperscribe.scribe.application.transcript_app.LaunchModalEffect")
def test_handle_ignores_non_true_secret(
    launch_modal_effect: object, mock_note: MagicMock, mock_hide: MagicMock
) -> None:
    """Only a strict "true" switches hiding on, so "false" really disables it."""
    launch_modal_effect.return_value.apply.side_effect = [  # type: ignore[union-attr]
        Effect(type="LOG", payload="SomePayload")
    ]
    launch_modal_effect.TargetType.NOTE = "note"  # type: ignore[union-attr]
    ...
    event = Event(EventRequest(context='{"note_id":5481}'))
    tested = ScribeApp(event, {"ScribeHideChartButtons": "false"})
    result = tested.handle()
