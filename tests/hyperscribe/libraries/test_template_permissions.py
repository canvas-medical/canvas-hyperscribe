...
def test_del_cleans_up_permissions():
...
def test_del_no_error_when_missing():
    TemplatePermissions.PERMISSIONS.clear()
    tested = TemplatePermissions("test-note-uuid")
    del tested
    assert "test-note-uuid" not in TemplatePermissions.PERMISSIONS
...
def test_load_permissions_success(mock_cache_getter):
    TemplatePermissions.PERMISSIONS.clear()
...
@patch.object(TemplatePermissions, "default_cache_getter")
def test_load_permissions_empty(mock_cache_getter):
    TemplatePermissions.PERMISSIONS.clear()
    mock_cache = MagicMock()
    mock_cache.get.side_effect = [{}]
    mock_cache_getter.side_effect = [mock_cache]
...
@patch.object(TemplatePermissions, "default_cache_getter")
def test_load_permissions_caches_result(mock_cache_getter):
    TemplatePermissions.PERMISSIONS.clear()
    mock_cache = MagicMock()
...
def test_load_permissions_import_error():
    TemplatePermissions.PERMISSIONS.clear()
    with patch.object(
...
def test_load_permissions_exception():
    TemplatePermissions.PERMISSIONS.clear()
    with patch.object(
        TemplatePermissions, "default_cache_getter", side_effect=RuntimeError("Cache unavailable")
    ) as mock_cache_getter:
        tested = TemplatePermissions("test-note-uuid")
        result = tested.load_permissions()
        expected = {}
...
@patch.object(TemplatePermissions, "default_cache_getter")
def test_multiple_notes_separate_caches(mock_cache_getter):
    TemplatePermissions.PERMISSIONS.clear()
    mock_cache = MagicMock()
    mock_cache.get.side_effect = [
...
@patch.object(TemplatePermissions, "default_cache_getter")
def test_shared_class_cache_single_load(mock_cache_getter):
    TemplatePermissions.PERMISSIONS.clear()
    mock_cache = MagicMock()
