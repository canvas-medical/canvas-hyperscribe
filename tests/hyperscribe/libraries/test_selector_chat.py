...
def test_condition_from(search_conditions):
    ...
    result = tested.condition_from(instruction, chatter, keywords[:3], keywords[3:], "theComment")
...
def test_lab_test_from(limited_cache):
    ...
    expressions = ["word1 word2 word3", " word4  "]
    ...
    assert result == expected
    calls = [call.lab_tests("theLabPartner", ["word1", "word2", "word3"]), call.lab_tests("theLabPartner", ["word4"])]
...
def test_contact_from(search_contacts, summary_of):
    ...
    system_prompt = [
        "The conversation is in the medical context.",
        "",
        "Your task is to identify the most relevant contact in regards of a search specialist out of a "
        "list of contacts.",
        "",
    ]
    user_prompt = [
    ...
    summaries = ["theSummary1", "theSummary2", "theSummary3"]
    instruction = Instruction(
        uuid="theUuid",
        index=0,
        instruction="theInstruction",
    ...
    result = tested.contact_from(instruction, chatter, "theFreeTextInformation", ["zip1", "zip2"])
    ...
    assert result == expected
    calls = [call("theFreeTextInformation", ["zip1", "zip2"])]
