...
from hyperscribe.handlers.transcript_display import TranscriptDisplay
...
def helper_instance() -> TranscriptDisplay:
    event = Event(
        EventRequest(
            context=json.dumps(
                {
    ...
    instance = TranscriptDisplay(event, secrets, environment)
...
@patch("hyperscribe.handlers.transcript_display.render_to_string")
@patch("hyperscribe.handlers.transcript_display.AwsS3")
def test_get(aws_s3, render_to_string):
    ...
    calls = [call("templates/transcript.html", {"url_list": "[]"})]
    ...
    calls = [
        ...
        call().is_ready(),
        call().list_s3_objects("hyperscribe-theTestEnv/transcripts/theNoteId/"),
