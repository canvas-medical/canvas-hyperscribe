...
from hyperscribe_tuning.handlers.limited_cache import LimitedCache, CodedItem
...
@patch('hyperscribe_tuning.handlers.limited_cache.date')
...
def test_demographic__str__(patient_db, observation_db, mock_date):
    ...
    tests = [
        ...
        (
            "F",
            date(2000, 2, 7),
            "the patient is a woman, born on <DOB REDACTED> (age 24) and weight 124.38 pounds"
        ),
        (
            "F",
            date(2020, 2, 7),
            "the patient is a girl, born on <DOB REDACTED> (age 4) and weight 124.38 pounds"
        ),
        (
            "F",
            date(2024, 7, 2),
            "the patient is a baby girl, born on <DOB REDACTED> (age 7 months) and weight 124.38 pounds"
        ),
        (
            "O",
            date(1941, 2, 7),
            "the patient is a elderly man, born on <DOB REDACTED> (age 83) and weight 124.38 pounds"
        ),
        (
            "O",
            date(2000, 2, 7),
            "the patient is a man, born on <DOB REDACTED> (age 24) and weight 124.38 pounds"
        ),
        (
            "O",
            date(2020, 2, 7),
            "the patient is a boy, born on <DOB REDACTED> (age 4) and weight 124.38 pounds"
        ),
        (
            "O",
            date(2024, 7, 2),
    ...
    tested = LimitedCache("patientUuid", {})
    result = tested.demographic__str__()
    expected = "the patient is a woman, born on <DOB REDACTED> (age 24)"
    assert result == expected
    assert tested._demographic == expected
    calls = [call.get(id="patientUuid")]
    assert patient_db.mock_calls == calls
    ...
    tested = LimitedCache("patientUuid", {})
    result = tested.demographic__str__()
    expected = "the patient is a woman, born on <DOB REDACTED> (age 24) and weight 125.00 pounds"
    assert result == expected
    assert tested._demographic == expected
    calls = [call.get(id="patientUuid")]
    assert patient_db.mock_calls == calls
