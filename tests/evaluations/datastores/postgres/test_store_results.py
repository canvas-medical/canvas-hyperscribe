...
def test_insert(alter, mock_datetime):
    ...
    exp_sql = ('INSERT INTO "results" ("created", "run_uuid", "plugin_commit", "case_type", "case_group", "case_name",'
...
def test_statistics_per_test(select):
    ...
    exp_sql = ('SELECT "case_name", "test_name",'
...
def test_statistics_end2end(select):
    ...
    assert len(select.mock_calls) == 1
    sql, params = select.mock_calls[0].args
    exp_sql = ('SELECT "case_name", SUM("full_run") AS "full_run", SUM("full_passed") AS "end2end", COUNT(distinct "run_uuid") AS "run_count" '
