...
def test_upsert(select, alter, mock_datetime):
    ...
    case = SyntheticCaseRecord(
        ...
        turn_total=10,
        speaker_sequence="CPCPCPCPCP",
        clinician_to_patient_turn_ratio=1.0,
        mood=SyntheticCaseMood.FRUSTRATED,
        pressure=SyntheticCasePressure.HIGH,
        clinician_style=SyntheticCaseClinicianStyle.FAST,
        patient_style=SyntheticCasePatientStyle.CURIOUS,
    ...
    assert params == {"case_id": 999}

    assert len(alter.mock_calls) == 1
    sql, params, involved_id = alter.mock_calls[0].args
    exp_sql = (
        'INSERT INTO "synthetic_case" ("created", "updated", "case_id", "category", "turn_total", '
        '"speaker_sequence", "clinician_to_patient_turn_ratio", "mood", "pressure", '
        '"clinician_style", "patient_style", "turn_buckets", "text_llm_vendor", "text_llm_name") '
        "VALUES (%(now)s, %(now)s, %(case_id)s, %(category)s, %(turn_total)s, %(speaker_sequence)s, "
        "%(clinician_to_patient_turn_ratio)s, %(mood)s, %(pressure)s, %(clinician_style)s, "
        "%(patient_style)s, %(turn_buckets)s, %(text_llm_vendor)s, %(text_llm_name)s) RETURNING id"
    )
    assert compare_sql(sql, exp_sql)
    assert involved_id is None
    assert params["now"] == date_0
    reset_mock()
    ...
    exp_sql = (
        ...
        '"speaker_sequence" = %(speaker_sequence)s, "clinician_to_patient_turn_ratio" = %(clinician_to_patient_turn_ratio)s, '
